...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function getCurrentSapUser() {
        // 1) dari meta
        const m = document
            .querySelector('meta[name="sap-user"]')
            ?.content?.trim();
        if (m) return m;

        // 2) fallback: dari window.SAP_AUTH = "Basic base64(user:pass)"
        try {
            const auth = String(window.SAP_AUTH || "");
            const mt = /^Basic\s+([A-Za-z0-9+/=]+)$/i.exec(auth);
            if (!mt) return "";
            const decoded = atob(mt[1] || "");
    ...
    function isLikelyEANFormat(fmt) {
        return /^ean(_\d+)?$/i.test(String(fmt || ""));
    }
    ...
    function addAufnrToList(aufnr) {
        ...
        div.className =
            "px-3 py-1.5 bg-slate-100 rounded-xl flex items-center justify-between text-xs font-medium text-slate-700 transition-all duration-200 hover:bg-slate-200";
    ...
    async function ajaxSync(payload) {
        ...
        try {
            ...
            if (res.status === 404) {
                return {
                    ok: false,
                    teco: !!json.teco_possible,
                    msg: json.message || "Data Tidak Ditemukan",
                    raw: json,
                };
            }
            if (!res.ok) {
            ...
            return {
                ok: true,
                received: Number(json?.received || json?.count || 0),
                raw: json,
            };
        } catch (e) {
    ...
    function showTeco(listOrText) {
        ...
        const text = Array.isArray(listOrText)
            ? listOrText.join(", ")
            : String(listOrText || "");
            ...
            try {
                ...
                if (hasAufnr) {
                    ...
                    if (failures.length > 0) {
                        ...
                        showErrorAndThen(
                            "Sebagian PRO gagal memuat data",
                            lines,
                            goRedirect
                        );
                        return;
                    ...
                    if (!r.ok) {
                        if (r.teco) showTeco(`${arbpl} - ${werks}`);
                            ...
                            showError(
                                "Data Tidak Ditemukan",
                                r.msg || "WC/Plant tidak mengembalikan data."
                            );
                        return;
                    }
                    const to = new URL(form.action, location.origin);
                ...
                showError(
                    ...
                    err?.message || "Terjadi kesalahan saat sinkronisasi."
                );
            } finally {
                setBusy(false);
            }
    ...
    async function startQrScanner() {
        ...
        if (isIOS) {
            ...
            try {
                ...
                const scanLoop = () => {
                    if (video.readyState === video.HAVE_ENOUGH_DATA) {
                        ...
                        if (code) {
                            const normalizedText = code.data
                                .trim()
            ...
            if (typeof Html5Qrcode === "undefined") {
                showError(
                    "Scanner QR tidak tersedia",
                    "Library html5-qrcode belum dimuat."
                );
                closeQrModal();
                return;
            }
    ...
    function closeLogout() {
        ...
        if (logoutConfirm) {
            ...
            if (logoutConfirm.dataset._txt)
                logoutConfirm.innerHTML = logoutConfirm.dataset._txt;
        }
    ...
    (function () {
        ...
        if (oldCaret && oldCaret.tagName?.toLowerCase() === "svg")
        ...
        menu.id = "plantMenu";
        menu.className =
            "dropdown-enter invisible opacity-0 scale-95 absolute right-0 mt-1 z-30 w-32 bg-white rounded-xl shadow-2xl ring-1 ring-slate-200 overflow-hidden";
        menu.setAttribute("role", "listbox");
        menu.tabIndex = -1;

        const header = document.createElement("div");
        header.className =
            "px-2 py-0.5 text-[10px] font-semibold tracking-wider text-slate-500 bg-slate-50";
      ...
      `;
        ...
        function showMenu() {
            ...
            const cur = items().findIndex(
                (li) => li.dataset.value === select.value
            );
        ...
        function setActive(i) {
            const arr = items();
            arr.forEach((li, idx) =>
                li.classList.toggle("dd-opt-focus", idx === i)
            );
            activeIdx = i;
    ...
    function fmtDateTime(v) {
        ...
        const jam = `${pad2(d.getHours())}:${pad2(d.getMinutes())}:${pad2(
            d.getSeconds()
        )}`;
        return `${tgl} ${jam}`;
    }
    ...
    function fmtQtyByUom(val, uomRaw) {
        const u = mapUom(uomRaw || "");
        const num = Number(val);
        if (!isFinite(num)) return "-";
        if (u === "PC") {
            return String(Math.round(num)); // tampilkan bulat
        }
        // tampilkan s.d. 3 desimal, buang nol buntut & titik jika tak perlu
    ...
    // ===== MONITOR KONFIRMASI (baru) =====
    const monitorBody = document.getElementById("monitorBody");
    const monitorMeta = document.getElementById("monitorMeta");
    const monitorRefreshBtn = document.getElementById("monitorRefresh");

    // ambil NIK dari querystring atau input
    const pernrFromQS =
        (new URLSearchParams(location.search).get("pernr") || "").trim() ||
        document.getElementById("IV_PERNR")?.value?.trim() ||
        "";
    ...
    function badgeStatus(s) {
        ...
        if (st === "SUCCESS")
            return `<span class="${base} bg-emerald-100 text-emerald-700">SUCCESS</span>`;
        if (st === "FAILED")
...
`;
    ...
    function startMonitor() {
        ...
        monitorTimer = setInterval(() => {
            if (!document.hidden) loadMonitor();
        }, 4000); // 4 detik
    }
    ...
    document.addEventListener("visibilitychange", () => {
        document.hidden ? stopMonitor() : startMonitor();
    });
...
(function () {
    ...
    const obs = new MutationObserver(() => {
        if (ov.classList.contains("hidden")) stop();
        else start();
    });
